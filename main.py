...
while z<=10:
  ...
  for g in a:
    ...
    def xor2(a2,b2):
        xored = []
        for i in range(max(len(a2), len(b2))):
            xored_value = ord(a2[i%len(a2)]) ^ ord(b2[i%len(b2)])
...
L=B
H="20"
# H=list(map(chr, range(97, 123)))
# H='qwertyuiopasdfghjklzxcvbnm'


def xor8(L,H):
    xored = []
    for V in range(max(len(L), len(H))):
        xored_value = ord(L[V%len(L)]) ^ ord(H[V%len(H)])
        xored.append(hex(xored_value)[2:])
    return ''.join(xored)
print("Строчно без пробела=","\n",xor8(L,H),"\n")
Result5=xor8(L,H)


L2=Result5

H2=list(map(chr, range(97, 123)))
# H='qwertyuiopasdfghjklzxcvbnm'


def xor22(L2,H2):
    xored = []
    for V2 in range(max(len(L2), len(H2))):
        xored_value = ord(L2[V2%len(L2)]) ^ ord(H2[V2%len(H2)])
        xored.append(hex(xored_value)[2:])
    return ''.join(xored)
print("Ответ=","\n",xor22(L2,H2))
Result5=xor22(L2,H2)